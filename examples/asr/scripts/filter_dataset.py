...
def main(input_file_path, output_file_path, num_proc=32):
    ...
    if "multilingual_librispeech" in input_file_path:
        bad_texts = {
            "la fable de",
            "à la maison du",
            "de m de",
        ...
        # normalize back to compare with bad texts
        # def tmp_normalize(s):
        #     s = s.lower()
        #     s = re.sub(r"[^a-zàâäéèêëîïôöùûüÿçñA-ZÀÂÄÇÈÉÊËÎÏÔÖÙÛÜŸ0-9]", "", s)
        #     s = re.sub(r"\s+", " ", s).strip()  # replace any successive whitespace characters with a space
        #     return s

        dataset = dataset.filter(lambda x: x["text"] not in bad_texts, num_proc=num_proc)
    ...
    print(f"The filtered data is saved into {output_file_path}")
