...
def prepare_output_dirs(cfg: HFDatasetConversionConfig):
    ...
    output_dir = os.path.join(output_dir, cfg.pretty_path)
...
def build_map_dataset_to_nemo_func(cfg: HFDatasetConversionConfig, basedir):
    ...
    def map_dataset_to_nemo(batch):
        ...
        if cfg.streaming:
            ...
            # bh: patch for local files
            if cfg.local_file is not None:
                segments = segments[-1:]
                # segments = segments[-2:]  # for lingualibre
                segments.insert(0, "audios")
...
def convert_offline_dataset_to_nemo(
    ...
    # print(dataset)
    # dataset = dataset.remove_columns(["audio", "file"])
    # # if "file" in dataset.features:
    # #     dataset = dataset.remove_columns("audio")
    # dataset.to_json(manifest_filepath, orient="records", lines=True, force_ascii=cfg.ensure_ascii)
    ...
    with open(manifest_filepath, 'w') as manifest_f:
        for idx, sample in enumerate(
            ...
            # allowed_keys = {"audio_filepath", "text", "duration"}
            # sample = {k: v for k, v in sample.items() if k in allowed_keys}
...
def process_dataset(dataset: IterableDataset, cfg: HFDatasetConversionConfig):
    ...
    if cfg.text_column_name is not None and cfg.text_column_name != "text":
        dataset = dataset.rename_column(cfg.text_column_name, "text")
    if "text" not in dataset.features:
        raise ValueError("Didn't find `text` column in dataset")

    if cfg.split_output_dir is None:
        basedir = cfg.resolved_output_dir
        # manifest_filename = f"{cfg.path.replace('/', '_')}_manifest.json"
        manifest_filename = f"{cfg.pretty_path.replace('/', '_')}_manifest.json"
    else:
        ...
        manifest_filename = f"{split}_{cfg.pretty_path.replace('/', '_')}_manifest.json"
...
def main(cfg: HFDatasetConversionConfig):
    ...
    try:
        if cfg.local_file is not None:
            # bh: load from local files
            dataset = load_dataset("csv", data_files=cfg.local_file, sep="\t")["train"]
            # bh: for mtedx
            # dataset = load_dataset("csv", data_files=cfg.local_file, sep="\t", escapechar="\\", quoting=csv.QUOTE_NONE)["train"]
        else:
